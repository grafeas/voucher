...
func TestCreateOccurrence(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"err status code": {
			...
			expectedErr: "error getting REST data with status code 500 for url /grafeastest/occurrences and method POST",
		},
	}
...
func TestListNotes(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"err status code": {
			...
			expectedNotes: notesR.Notes,
			statusCode:    http.StatusInternalServerError,
			expectedErr:   "error getting REST data with status code 500 for url /grafeastest/notes and method GET",
		},
	}
	for tc, test := range tcs {
		t.Run(tc, func(t *testing.T) {
...
func TestListOccurrences(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"err status code": {
			...
			expectedOccs: occsR.Occurrences,
			statusCode:   http.StatusInternalServerError,
			expectedErr:  "error getting REST data with status code 500 for url /grafeastest/occurrences and method GET",
		},
	}
	for tc, test := range tcs {
		t.Run(tc, func(t *testing.T) {
